...
const createSong = async (song) => {
  try {
    if (!song.name) {
      throw 'You must specify a value "name"';
    }
    const newSong = await db.one(
      "INSERT INTO songs (name, artist, album, duration, is_favorite) VALUES ($1, $2, $3, $4, $5) RETURNING *",
      [song.name, song.artist, song.album, song.duration, song.is_favorite]
    );
    return newSong;
  } catch (error) {
    return error;
...
const updateSong = async (id, song) => {
  try {
    const updatedSong = await db.one(
      ...
      [song.name, song.artist, song.album, song.duration, song.is_favorite, id]
    );
    return updatedSong;
  } catch (error) {
    return error;
